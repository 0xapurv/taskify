...
import 'package:taskify/viewmodels/task_view_model.dart';
...
void main() {
  group('TaskViewModel Tests', () {
    ...
    test('should filter tasks correctly', () async {
      ...
      // Complete one task
      final firstTaskId = viewModel.tasks.first.taskId;
      ...
      expect(viewModel.tasks.length, 2);
      expect(viewModel.tasks.every((task) => !task.isTaskCompleted), true);
    ...
    test('should toggle task completion correctly', () async {
      ...
      await viewModel.toggleTaskCompletion(task.taskId);
      expect(viewModel.tasks.first.isTaskCompleted, true);
      
      await viewModel.toggleTaskCompletion(task.taskId);
      expect(viewModel.tasks.first.isTaskCompleted, false);
    });
    ...
    test('should delete tasks correctly', () async {
      ...
      final taskId = viewModel.tasks.first.taskId;
